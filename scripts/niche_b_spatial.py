"""
###############################################################################
SPATIAL XENIUM 5K PIPELINE — niche_b  (revised)
Per-slide spatial maps: cell types, niches, and condition
###############################################################################
...
###############################################################################
  ...
  * Condition added to the spatial view:
        ...
        - condition-high-cell highlight grid (cells above the shared top-quartile
          threshold lit up), TEST arranged above CNTRL.
  * A compact condition-by-niche summary (TEST vs CNTRL, shared niche colours,
...
###############################################################################
    ...
    condition_score_grid.pdf        continuous condition score on tissue
    condition-high_cells_grid.pdf         condition-high-cell highlight on tissue
    condition_by_niche_summary.pdf  per-niche TEST vs CNTRL (mixed-model stars)
###############################################################################
"""
...
OUTPUT_DIR = _ARGS.output_dir or "spatial_maps"
...
FIG_DPI        = 150
SCORE_CMAP     = "magma"
CONDITION_HIGH_COLOR = "#FFD23F"     # condition-high-cell highlight
...
# conditions present, ordered TEST-above-CNTRL
...
n_test = len(samples_by_cond.get("TEST", []))
n_cntrl = len(samples_by_cond.get("CNTRL", []))
...
def condition_grid(draw_fn, out_path, suptitle, facecolor="white",
                   ...
                   label_color=None):
    """draw_fn(ax, sub) renders one slide. Rows = conditions (TEST top), cols =
...
if HAS_COND:
    ...
    print("[PLOT] Condition-high-cell highlight grid...")

    def _draw_condition_high(ax, sub):
        coords = np.asarray(sub.obsm[SPATIAL_KEY])
        ...
        if (~flag).any():
            ax.scatter(coords[~flag, 0], coords[~flag, 1], c=BG_POINT_COLOR,
                       ...
                       linewidths=0)
        if flag.any():
            ax.scatter(coords[flag, 0], coords[flag, 1], c=CONDITION_HIGH_COLOR,
                       s=POINT_SIZE * 1.3, alpha=POINT_ALPHA, rasterized=True,
                       linewidths=0)

    q = int(CONDITION_SCORE_QUANTILE * 100)
    condition_grid(_draw_condition_high,
                   os.path.join(OUTPUT_DIR, "condition-high_cells_grid.pdf"),
                   f"Condition-high cells (top {100-q}% condition, shared threshold)  ·  "
                   f"{N_NOTE}", facecolor=BG_COLOR,
                   legend_handles=[plt.Line2D([0], [0], marker="o", color="w",
                                   markerfacecolor=CONDITION_HIGH_COLOR, markersize=8,
                                   label=f"condition-high (≥ q{q})")])

    # 6b. condition-by-niche summary (mixed-model stars)
    print("[PLOT] Condition-by-niche summary...")
    ...
    for niche in all_niches:
        ...
        rows.append({"niche": niche, "mean_score": d["score"].mean(),
                     "mean_TEST": d[d.condition == "TEST"]["score"].mean(),
                     "mean_CNTRL": d[d.condition == "CNTRL"]["score"].mean(),
                     **res})
    ...
    cond_niche = cond_niche.sort_values("mean_score", ascending=False)
    cond_niche.to_csv(os.path.join(OUTPUT_DIR, "condition_by_niche_summary.csv"),
                     index=False)

    order = cond_niche["niche"].tolist()
    ...
    for gi, niche in enumerate(order):
        ...
        star = pval_stars(cond_niche.set_index("niche").loc[niche, "padj"])
        if star:
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(OUTPUT_DIR, "condition_by_niche_summary.pdf"))
    plt.close()

###############################################################################
# 7. DONE
###############################################################################
print("\n[DONE] niche_c complete. Outputs in spatial_maps/")
print(f"  Cell-type and niche colours are shared with 2.5a / 2.5b. Condition")

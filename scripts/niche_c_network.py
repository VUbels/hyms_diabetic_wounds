"""
###############################################################################
SPATIAL XENIUM 5K PIPELINE — niche_c  (revised)
...
###############################################################################
  * Shared niche colour map (same niche = same colour as in 2.5a / 2.5c) and a
    shared Kamada-Kawai layout, so the TEST and CNTRL networks are directly
  ...
  * Neighbourhood enrichment (squidpy) added as the field-standard niche-
    ...
    with TEST / CNTRL / Δ heatmaps. Proximity (closest-approach distance) is kept as
  ...
  * Optional condition-coloured network: nodes shaded by mean condition score on
    the shared layout, to reveal condition-high niche hubs in their spatial context.
  * Significance markers / captions state the test and the n explicitly.

Sample size note: with n=3 TEST vs n=2 CNTRL slides, the minimum achievable exact
...
###############################################################################
    niche_network_{TEST,CNTRL,combined}.pdf          per-condition + overlaid (shared layout)
    niche_architecture_change.pdf               prevalence shift vs neighbour rewiring
    niche_egocentric_change.pdf                 CNTRL-vs-TEST neighbours for most-changed niches
    niche_network_condition.pdf
    ...
    niche_nhood_enrichment_{TEST,CNTRL,delta}.csv
###############################################################################
"""
...
OUTPUT_DIR = _ARGS.output_dir or "niche_network"
...
# Condition score (for the condition-coloured network) + shared styles.
...
HAS_COND = (CONDITION_SCORE_KEY in adata.obs
...
n_test = sum(1 for c in slide_cond.values() if c == "TEST")
n_cntrl = n_slides - n_test
...
n_arr = comb(n_slides, n_test)
use_exact = n_arr <= N_PERMUTATIONS
if use_exact:
    all_test_assignments = list(combinations(range(n_slides), n_test))
    print(f"  exact permutation: {len(all_test_assignments)} arrangements")
else:
    rng = np.random.default_rng(42)
    all_test_assignments = [tuple(sorted(rng.choice(n_slides, n_test, replace=False)))
...
def _perm_p(slide_to_val, observed_abs):
    n_ge = tot = 0
    for combo in all_test_assignments:
        test_s = [slides_list[i] for i in combo]
        hv = [slide_to_val[s] for s in test_s if s in slide_to_val]
        lv = [slide_to_val[s] for s in slides_list
              if s not in test_s and s in slide_to_val]
        if not hv or not lv:
            continue
...
def compute_edge_stats(pair_df):
    hv = pair_df[pair_df.condition == "TEST"]["proximity_p25"].values
    lv = pair_df[pair_df.condition == "CNTRL"]["proximity_p25"].values
    hc = pair_df[pair_df.condition == "TEST"]["contact_freq"].values
    lc = pair_df[pair_df.condition == "CNTRL"]["contact_freq"].values
    out = {"TEST_mean": hv.mean() if len(hv) else np.nan,
           "CNTRL_mean": lv.mean() if len(lv) else np.nan,
           "n_TEST": len(hv), "n_CNTRL": len(lv),
           "delta_proximity": np.nan,
           "TEST_contact_freq": hc.mean() if len(hc) else np.nan,
           "CNTRL_contact_freq": lc.mean() if len(lc) else np.nan,
...
test_mat = build_matrix(prox_by_cond[prox_by_cond.condition == "TEST"],
                      all_niches, "mean_proximity")
cntrl_mat = build_matrix(prox_by_cond[prox_by_cond.condition == "CNTRL"],
                      all_niches, "mean_proximity")
np.fill_diagonal(test_mat.values, 0.0)
np.fill_diagonal(cntrl_mat.values, 0.0)
...
print("\n[MANTEL] TEST vs CNTRL proximity-matrix concordance...")
mantel_r, mantel_p, n_pairs = mantel_test(test_mat, cntrl_mat, n_perm=N_PERMUTATIONS)
...
with open(os.path.join(OUTPUT_DIR, "mantel_test_result.txt"), "w") as f:
    f.write("Mantel test: TEST vs CNTRL niche proximity matrices\n")
...
def global_architecture_permutation(per_slide_df, metric_col):
    ...
    nhsi = int((labels == "TEST").sum())
    perms = list(combinations(range(len(slides)), nhsi))
    cf = cd = tot = 0
    for combo in perms:
        lab = np.array(["CNTRL"] * len(slides), dtype=object)
        for i in combo:
            lab[i] = "TEST"
...
with open(os.path.join(OUTPUT_DIR, "global_architecture_test.txt"), "w") as f:
    ...
    f.write("permutation (slide = unit). Larger F / smaller p => TEST and CNTRL slides\n")
    ...
    f.write(f"\nNOTE: with {N_NOTE} the smallest achievable permutation p is "
            f"{1.0/max(comb(n_slides, n_test),1):.2f}.\n")
if glob_results:
...
vh, vl = test_mat.values[iu], cntrl_mat.values[iu]
...
ax.set_xlabel("CNTRL proximity (µm)"); ax.set_ylabel("TEST proximity (µm)")
gp = glob_results[0]["perm_p_F"] if glob_results else np.nan
ax.set_title(f"Global niche architecture: TEST vs CNTRL\nMantel r={mantel_r:.3f}, "
    ...
    if nhood:
        ...
        if all(c in nhood for c in CONDITIONS):
            ...
            sns.heatmap(pd.DataFrame(dz.values, index=lbl, columns=lbl),
                        ...
                        linewidths=0.3, cbar_kws={"label": "Δ z (TEST−CNTRL)"})
            axes[-1].set_title("TEST − CNTRL")
...
sns.heatmap(delta_display, cmap="RdBu", center=0, vmin=-abs_max, vmax=abs_max,
            cbar_kws={"label": "Δ proximity TEST−CNTRL (µm)  ·  red: closer in TEST"},
...
if len(diff_top):
    ...
    plt.colorbar(sm, ax=ax, fraction=0.04, pad=0.04,
                 label="Δ proximity TEST−CNTRL (µm)  ·  alpha ∝ −log10(perm p)")
...
###############################################################################
# 11. CONDITION-COLOURED NETWORK (nodes shaded by mean condition score)
###############################################################################
if HAS_COND:
    print("[PLOT] Condition-coloured network...")
    score_by_niche = adata.obs.groupby(NICHE_KEY)[CONDITION_SCORE_KEY].mean()
    vals = np.array([score_by_niche.get(n, np.nan) for n in all_niches], dtype=float)
    finite = vals[np.isfinite(vals)]
    if len(finite):
        norm = mpl.colors.Normalize(vmin=float(np.nanmin(finite)),
                                    vmax=float(np.nanmax(finite)))
        cmap = plt.cm.magma
        facecol = {n: cmap(norm(score_by_niche.get(n, np.nan)))
                   if np.isfinite(score_by_niche.get(n, np.nan)) else "#DDDDDD"
                   for n in all_niches}
        fig, ax = plt.subplots(figsize=(10, 9))
        ...
        ax.set_title("Condition-high niche hubs on the shared spatial layout\n"
                     "(node colour = mean condition; edges = pooled proximity)",
                     fontsize=10)
        plt.tight_layout()
        plt.savefig(os.path.join(OUTPUT_DIR, "niche_network_condition.pdf"))
        plt.close()
...
for ax, cond in zip(axes[:-1], CONDITIONS):
    ...
    ax.set_title(COND_SHORT[cond]); plt.setp(ax.get_xticklabels(), rotation=45,
                                             ha="right")
sns.heatmap(delta_display, cmap="RdBu", center=0, vmin=-abs_max, vmax=abs_max,
            ax=axes[-1], cbar_kws={"label": "TEST−CNTRL proximity (µm)"})
axes[-1].set_title("TEST − CNTRL"); plt.setp(axes[-1].get_xticklabels(), rotation=45,
                                        ha="right")
...
test_c = build_contact_matrix(prox_by_cond[prox_by_cond.condition == "TEST"],
                            all_niches)
cntrl_c = build_contact_matrix(prox_by_cond[prox_by_cond.condition == "CNTRL"],
                            all_niches)
dc = test_c - cntrl_c
...
sns.heatmap(dc_disp, cmap="RdBu_r", center=0, vmin=-abs_c, vmax=abs_c, ax=axes[-1],
            cbar_kws={"label": "Δ contact freq (TEST−CNTRL)"})
axes[-1].set_title("TEST − CNTRL"); plt.setp(axes[-1].get_xticklabels(), rotation=45,
                                        ha="right")
...
test_slides_n = [s for s in slides_list if slide_cond[s] == "TEST"]
cntrl_slides_n = [s for s in slides_list if slide_cond[s] == "CNTRL"]
...
arch_rows = []
for n in all_niches:
    mh = float(prop_tab_net.loc[n, test_slides_n].mean())
    ml = float(prop_tab_net.loc[n, cntrl_slides_n].mean())
    nh = int(ncell_cond.loc[n].get("TEST", 0))
    nl = int(ncell_cond.loc[n].get("CNTRL", 0))
    both = nh >= ARCH_MIN_CELLS and nl >= ARCH_MIN_CELLS
    h = test_c.loc[n].drop(n).values.astype(float)
    l = cntrl_c.loc[n].drop(n).values.astype(float)
    ...
    arch_rows.append({"niche": n, "mean_prop_TEST": mh, "mean_prop_CNTRL": ml,
                      "abund_log2fc": float(np.log2((mh + 1e-4) / (ml + 1e-4))),
                      "n_TEST": nh, "n_CNTRL": nl, "present_both": both,
                      "rewiring_cosine": rw})
...
ya = range(len(a))
ca = [COND_COLORS["TEST"] if v > 0 else COND_COLORS["CNTRL"]
...
axA.axvline(0, color="0.6", lw=0.8); axA.set_xlabel("abundance log2(TEST/CNTRL)")
axA.set_title("Prevalence shift\n(how often the niche occurs: TEST vs CNTRL)", fontsize=10)
for s_ in ("top", "right"):
    axA.spines[s_].set_visible(False)
b = arch[arch["rewiring_cosine"].notna()].sort_values("rewiring_cosine")
yb = range(len(b))
cb = [COND_COLORS["TEST"] if v > 0 else COND_COLORS["CNTRL"]
...
fig.suptitle(f"Niche architecture change between conditions  ·  {N_NOTE}\n"
             ...
             "neighbours change · bar colour = TEST- (red) or CNTRL- (blue) enriched",
             fontsize=11)
plt.tight_layout()
...
if ego:
    ...
    for r, n in enumerate(ego):
        ...
        axs.text(0.1, 0.9, f"N{n}", fontsize=13, fontweight="bold", va="center",
                 transform=axs.transAxes)
        is_test = row["abund_log2fc"] > 0
        axs.text(0.0, 0.66, f"{'TEST' if is_test else 'CNTRL'}-enriched · "
                 f"log2FC {row['abund_log2fc']:+.1f}", fontsize=9,
                 color=COND_COLORS["TEST"] if is_test
                 else COND_COLORS["CNTRL"], transform=axs.transAxes)
        axs.text(0.0, 0.47, f"cells: TEST {int(row['n_TEST'])} · CNTRL {int(row['n_CNTRL'])}",
        ...
        lp, hp = top_partners(cntrl_c, n), top_partners(test_c, n)
        mx = max([v for _, v in lp] + [v for _, v in hp] + [0.01])
        for col, parts0, lab in [(1, lp, "CNTRL neighbours"), (2, hp, "TEST neighbours")]:
        ...
        if r == 0:
            axs.text(0.0, 1.08, "niche · prevalence · cells", fontsize=10,
                     transform=axs.transAxes)
    fig.suptitle(f"Egocentric niche neighbourhoods: CNTRL vs TEST  ·  {N_NOTE}\n"
...
# 13. DONE
###############################################################################
print("\n[DONE] niche_b complete. Outputs in niche_network/")
...
print(f"  (niche_egocentric_change.pdf) show CNTRL-vs-TEST neighbours for the most-changed.")
...
print(f"  Edge thresholds are POOLED so TEST/CNTRL panels are comparable. Colours and")
